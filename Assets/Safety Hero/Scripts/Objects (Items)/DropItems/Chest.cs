...
{
    [Header("# 상자 정보")]
    public static int basechestCost = 50;
    public static int openCount = 0;
    private float costMultiplier = 1.15f;

    // [핵심 1] "박스가 열렸다"는 사실을 알리는 방송국(이벤트)
    public static Action OnChestCostChanged;
    ...
    private void Start()
    { 
        OnChestCostChanged += UpdatePriceText;
        UpdatePriceText();
    }
    ...
    {
        float cost = basechestCost * Mathf.Pow(costMultiplier, openCount);
        return (int)cost;
    }
    ...
    public override void Interact()
    {
        ...
        if (playerStats.gold >= cost)
        {
            ...
            OnChestCostChanged?.Invoke();

            Debug.Log($"상자 오픈! 소모 골드: {cost}");
