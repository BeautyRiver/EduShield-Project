...
public class W1_RotateWeapon : Weapon, IBatchable, IRotatingable
{
    protected override void Update()
    {
        ...
        // 회전 로직 추가
        transform.Rotate(Vector3.back * finalStats.rotationSpeed * Time.deltaTime);
    }
    ...
    protected IEnumerator M1_Bullet()
    {
        ...
        if (finalStats.weaponDuration > 0f)
        {
            yield return new WaitForSeconds(finalStats.weaponDuration);
    ...
    public void Batch()
    {
        for (int index = 0; index < finalStats.count; index++) // 불릿 수만큼 반복
        {
            ...
            // 초기 회전 설정
            Vector3 rotVec = Vector3.forward * 360 * index / finalStats.count; // 불릿 회전 벡터 계산
            bullet.Rotate(rotVec); // 불릿 회전                        
            bullet.localScale = Vector3.zero;
            bullet.Translate(bullet.up * finalStats.attackRange, Space.World); // 지정된 거리만큼 이동               
            bullet.DOScale(finalStats.bulletSize, 0.5f).SetEase(Ease.OutBounce); // 크기를 0.5초 동안 자연스럽게 확장
    ...
    public void Rotate()
    {
        transform.Rotate(Vector3.back * finalStats.bulletDelay * Time.deltaTime); // 무기 회전
    }
}
